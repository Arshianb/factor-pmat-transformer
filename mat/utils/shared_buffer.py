...
class SharedReplayBuffer(object):
    ...
    def __init__(self, args, num_agents, obs_space, cent_obs_space, act_space, env_name):
        ...
        self.agent_ids = np.zeros((self.episode_length + 1, self.n_rollout_threads, num_agents, 1), dtype=np.float32)
        ...
        self.dones = np.zeros((self.episode_length + 1, self.n_rollout_threads, num_agents),
                                  dtype=np.float32)
                                  
        self.step = 0

    def insert(self, share_obs, obs, rnn_states_actor, rnn_states_critic, actions, action_log_probs, seq_log_probs, seqs,
               value_preds, rewards, masks, agent_ids, bad_masks=None, active_masks=None, available_actions=None):
        """
        ...
        self.agent_ids[self.step + 1] = agent_ids.copy()
    ...
    def chooseinsert(self, share_obs, obs, rnn_states, rnn_states_critic, actions, action_log_probs,
        ...
        self.agent_ids[self.step] = agent_ids.copy()
    ...
    def feed_forward_generator_transformer(self, advantages, num_mini_batch=None, mini_batch_size=None):
        ...
        obs = obs[rows, cols]
        agent_ids = self.agent_ids[:-1].reshape(-1, *self.agent_ids.shape[2:])
        agent_ids = agent_ids[rows, cols]
        

        rewards = self.rewards.reshape(-1, *self.rewards.shape[2:]) # CORRECTED: NO SLICING
        rewards = rewards[rows, cols]


        raw_next_obs = self.obs[1:].reshape(-1, *self.obs.shape[2:])
        next_obs = raw_next_obs[rows, cols]
        dones = self.dones[:-1].reshape(-1, *self.dones.shape[2:])
        dones_batch = dones[rows, cols]
        if len(dones_batch.shape) < len(next_obs.shape):
            dones_batch = np.expand_dims(dones_batch, -1)
        next_obs = next_obs * (1 - dones_batch)
        ...
        for indices in sampler:
            ...
            agent_ids_batch = agent_ids[indices].reshape(-1, *agent_ids.shape[2:])
            next_obs_batch = next_obs[indices].reshape(-1, *obs.shape[2:])
            reward_batch_raw = rewards[indices].reshape(-1, *rewards.shape[2:])
            ...
            yield share_obs_batch, obs_batch, next_obs_batch, reward_batch_raw, rnn_states_batch, rnn_states_critic_batch, actions_batch, \
