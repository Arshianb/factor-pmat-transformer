...
def shareworker(remote, parent_remote, env_fn_wrapper):
    ...
    while True:
        cmd, data = remote.recv()
        if cmd == 'step':
            ob, s_ob, reward, done, info, available_actions, agent_ids = env.step(data)
            if 'bool' in done.__class__.__name__:
                if done:
                    ob, s_ob, available_actions, agent_ids = env.reset()
            else:
                if np.all(done):
                    ob, s_ob, available_actions, agent_ids = env.reset()

            remote.send((ob, s_ob, reward, done, info, available_actions, agent_ids))
        elif cmd == 'reset':
            ob, s_ob, available_actions, agent_ids = env.reset()
            remote.send((ob, s_ob, available_actions, agent_ids))
        elif cmd == 'reset_task':
            ob = env.reset_task()
            remote.send(ob)
        elif cmd == 'render':
...
class ShareSubprocVecEnv(ShareVecEnv):
    ...
    def step_wait(self):
        results = [remote.recv() for remote in self.remotes]
        self.waiting = False
        obs, share_obs, rews, dones, infos, available_actions, agent_ids = zip(*results)
        return np.stack(obs), np.stack(share_obs), np.stack(rews), np.stack(dones), infos, np.stack(available_actions), np.stack(agent_ids)

    def reset(self):
        for remote in self.remotes:
            remote.send(('reset', None))
        results = [remote.recv() for remote in self.remotes]
        obs, share_obs, available_actions, agent_id = zip(*results)
        return np.stack(obs), np.stack(share_obs), np.stack(available_actions), np.stack(agent_id)
...
def chooseworker(remote, parent_remote, env_fn_wrapper):
    ...
    while True:
        ...
        if cmd == 'step':
            ...
            ob, s_ob, available_actions, agent_ids = env.reset(data)
            remote.send((ob, s_ob, available_actions, agent_ids))
        elif cmd == 'reset_task':
            ob = env.reset_task()
            remote.send(ob)
...
class ChooseSubprocVecEnv(ShareVecEnv):
    ...
    def reset(self, reset_choose):
        for remote, choose in zip(self.remotes, reset_choose):
            remote.send(('reset', choose))
        results = [remote.recv() for remote in self.remotes]
        obs, share_obs, available_actions, agent_ids = zip(*results)
        return np.stack(obs), np.stack(share_obs), np.stack(available_actions), np.stack(agent_ids)
...
class ShareDummyVecEnv(ShareVecEnv):
    ...
    def step_wait(self):
        results = [env.step(a) for (a, env) in zip(self.actions, self.envs)]
        obs, share_obs, rews, dones, infos, available_actions, agent_ids = map(
            np.array, zip(*results))

        for (i, done) in enumerate(dones):
            if 'bool' in done.__class__.__name__:
                if done:
                    obs[i], share_obs[i], available_actions[i], agent_ids[i] = self.envs[i].reset()
            else:
                if np.all(done):
                    obs[i], share_obs[i], available_actions[i], agent_ids[i] = self.envs[i].reset()
        self.actions = None

        return obs, share_obs, rews, dones, infos, available_actions, agent_ids

    def reset(self):
        results = [env.reset() for env in self.envs]
        obs, share_obs, available_actions, agent_ids = map(np.array, zip(*results))
        return obs, share_obs, available_actions, agent_ids
