...
class FootballEnv(MultiAgentEnv):
    ...
    def __init__(self, run_dir, **kwargs):
        ...
        if kwargs["env_args"]["eval"]:   
            dest_videos = str(run_dir) + "/" + str(kwargs["env_args"]["thread_num"])
    ...
    def reset(self, **kwargs):
        ...
        for obs_dict in obs_dicts:
            agent_id.append(obs_dict["active"])
            obs_i, ava_i = self._encode_obs(obs_dict)
            obs.append(obs_i)
            ava.append(ava_i)
        state = obs.copy()
        return obs, state, ava, agent_id
    ...
    def step(self, actions):
        ...
        return obs, state, rewards, dones, infos, ava, agent_id
