...
class Runner(object):
    ...
    def compute(self):
        ...
        if self.buffer.available_actions is None:
            next_values = self.trainer.policy.get_values(np.concatenate(np.expand_dims(self.buffer.agent_ids[-1], -1)),
    ...
    def restore(self, model_dir="", help_enc_model=""):
        """Restore policy's networks from a saved model."""
        if len(model_dir):
            self.policy.restore(model_dir)
        if len(help_enc_model):
            self.policy.restore_help_enc(help_enc_model)
