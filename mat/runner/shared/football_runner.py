...
class FootballRunner(Runner):
    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            for step in range(self.episode_length):
                ...
                obs, share_obs, rewards, dones, infos, available_actions, agent_ids = self.envs.step(actions)
                ...
                data = obs, share_obs, rewards, dones, infos, available_actions, agent_ids, \
    ...
    def warmup(self):
        # reset env
        obs, share_obs, ava, agent_ids = self.envs.reset()
        ...
        self.buffer.agent_ids[0] = np.expand_dims(agent_ids, -1).copy()

    @torch.no_grad()
    def collect(self, step):
        self.trainer.prep_rollout()
        value, action, action_log_prob, seq_log_prob, seq, rnn_state, rnn_state_critic \
            = self.trainer.policy.get_actions(np.concatenate(np.expand_dims(self.buffer.agent_ids[step], -1)),
    ...
    def insert(self, data):
        obs, share_obs, rewards, dones, infos, available_actions, agent_ids, \
        ...
        self.buffer.insert(share_obs, obs, rnn_states, rnn_states_critic,
                           actions, action_log_probs, seq_log_probs, seqs, values, rewards, masks, np.expand_dims(agent_ids,-1), None, active_masks,
                           available_actions)
    ...
    def eval(self, total_num_steps):
        ...
        eval_obs, eval_share_obs, ava, agent_ids = self.eval_envs.reset()
        ...
        while True:
            ...
            eval_actions, eval_rnn_states = \
                self.trainer.policy.act(np.concatenate(np.expand_dims(agent_ids,-1)),
            ...
            # Obser reward and next obs
            eval_obs, eval_share_obs, eval_rewards, eval_dones, eval_infos, ava, agent_ids = self.eval_envs.step(eval_actions)
