...
def another_discrete_autoregreesive_act(
        decoder,
        obs_rep,
        obs,
        batch_size,
        ...
        deterministic=False,
        factor_rep=None,
        agent2factor_mask=None,
        factor2agent_mask=None
):
    ...
    for i in range(n_agent):
        logit_full = decoder(
            ...
            agent2factor_mask=agent2factor_mask,
            factor2agent_mask=factor2agent_mask 
        )

        logit = logit_full[:, i, :] 

        if available_actions is not None:
            logit = logit.masked_fill(available_actions[:, i, :] == 0, -1e10)
...
def discrete_parallel_act(decoder, obs_rep, obs, action, batch_size, n_agent, action_dim, tpdv,
                          available_actions=None):
    action = action.to(action.device, dtype=torch.long)
...
def another_discrete_parallel_act(
        decoder,
        obs_rep,
        obs,
        action,
    ...
    action = action.to(action.device, dtype=torch.long)
    one_hot_action = F.one_hot(action.squeeze(-1), action_dim)
    ...
    if available_actions is not None:
        logit = logit.masked_fill(available_actions == 0, -1e10)
    distr = Categorical(logits=logit)
    action_log = distr.log_prob(action.squeeze(-1)).unsqueeze(-1)
