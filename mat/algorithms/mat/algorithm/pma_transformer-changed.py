...
class FactorAttention(nn.Module):
    ...
    def __init__(self, n_embd, n_head, n_agent, n_factor):
    ...
    def forward(self, node_x, factor_x, agent2factor_mask, factor2agent_mask):
        ...
        att_f2n = F.softmax(att_f2n, dim=-1)
...
class FactorBlock(nn.Module):
    ...
    def __init__(self, n_embd, n_head, n_agent, n_factor):
        ...
        self.attn = FactorAttention(n_embd, n_head, n_agent, n_factor)
...
class Encoder(nn.Module):
    def __init__(self, state_dim, obs_dim, n_block, n_embd, n_head,
        ...
        self.blocks = nn.ModuleList([
            FactorBlock(n_embd, n_head, n_agent, n_factor)
    ...
    def _build_factor_obs(self, obs, agent2factor_raw):
        B, N_agent, D = obs.size()
        mask = agent2factor_raw 
        denom = mask.sum(dim=0).clamp(min=1.0) 
        mask_exp = mask.view(1, N_agent, self.n_factor, 1) 
        obs_exp = obs.unsqueeze(2) 
        num = (obs_exp * mask_exp).sum(dim=1) 
        factor_obs = num / denom.view(1, self.n_factor, 1)
        return factor_obs

    def forward(self, state, obs, agent2factor_mask):
        agent2factor_raw = agent2factor_mask[0, :, :] 
        agent2factor_mask_bool = agent2factor_raw.bool().view(1, 1, self.n_agent, self.n_factor)
        ...
        factor_obs = self._build_factor_obs(obs, agent2factor_raw)
        ...
        for block in self.blocks:
            node_x, factor_x = block(node_x, factor_x, agent2factor_mask_bool, factor2agent_mask_bool)
...
class MultiAgentTransformer(nn.Module):
    def __init__(self, state_dim, obs_dim, action_dim, n_agent,
                 n_block, n_embd, n_head, n_factor, 
        ...
        self.encoder = Encoder(state_dim, obs_dim, n_block, n_embd, n_head, 
                               n_agent, n_factor, encode_state)
    ...
    def forward(self, state, obs, action, seq, agent2factor_mask, available_actions=None, deterministic=True):
        ...
        v_loc, obs_rep, factor_rep = self.encoder(state, obs, agent2factor_mask)
    ...
    def get_values(self, state, obs, agent2factor_mask, available_actions=None):
        ...
        state = check(state).to(**self.tpdv)
        obs = check(obs).to(**self.tpdv)
        v_tot, obs_rep, factor_rep = self.encoder(state, obs, agent2factor_mask)
        return v_tot
